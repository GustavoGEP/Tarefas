...
def receber_os_dados():
    ...
    return datas, session, sopa, response, headers, url
...
def recolhimento_dos_dados(df_data:pd.DataFrame, session, url, headers, df_lista:pd.DataFrame):
...
def tratamento_dos_dados(df:pd.DataFrame, df_lista:pd.DataFrame):
...
def baixar_excel(df:pd.DataFrame):
    ...
    df.to_excel(dados_excel, sheet_name="dados_excel2")
...
def main():
    ...
    dados, session, sopa, response, headers, url = receber_os_dados()
    
    df_data, df_lista = lista_datas(dados)
    
    df_lista, df = recolhimento_dos_dados(df_data, session, url, headers, df_lista)
    
    df = tratamento_dos_dados(df, df_lista)
